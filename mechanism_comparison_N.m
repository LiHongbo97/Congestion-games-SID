function [ output_args ] = mechanism_comparison_N(input_args)
...
alpha_h=2;bar_alpha=0.5; % Worst IR parameters
% alpha_h=10;
% bar_alpha=0.1;
...
% To summarize the IR with alpha_h=2
IR_m=[IR_m2 IR_m3 IR_m4 IR_m5];
IR_info=[IR_info12 IR_info13 IR_info14 IR_info15];
% To obtain another mechanism IR with alpha_h=10
[IR_m_2 IR_info_2]=mechanism_comparison_N1(5);
kx=[2 3 4 5];
% 
% figure % marker size: 16; font size: 24; legend font size: 20;
plot(kx(:),IR_m(:),'-bo','linewidth',1);hold on;
plot(kx(:),IR_m_2(:),'-go','linewidth',1);hold on;
plot(kx(:),IR_info(:),'-.r*','linewidth',1);hold on;
plot(kx(:),IR_info_2(:),'-.y*','linewidth',1);hold on;
xticks([2 3 4 5])
yticks([1 6 11 16])
% xlabel('Belief state x_i(t)');
xlabel('Risky path number N');
ylabel('Avg inefficiency ratios');
legend('\gamma^{(m)} with \alpha_H=2', '\gamma^{(m)}  with \alpha_H=5','\gamma^{(SID)} with \alpha_H=2', '\gamma^{(SID)} with \alpha_H=5');
