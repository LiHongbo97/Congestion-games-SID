function [ C,pi ] = C_opt_multi_comparison(L,l0,X,count,N,q_ll)
...
% 
% for i=1:10
%     q_ll(i)=1-(0.1*(i-1)*(1-q_hh))/(1-0.1*(i-1));
% end
...
if count>0
    [C1,pi1]=C_opt_multi_comparison(L_explore,l0*alpha,X_G,count-1,N,q_ll);
    [C2,pi2]=C_opt_multi_comparison(L_explore,l0*alpha,X_B,count-1,N,q_ll);
%     if N==2
%         P_G
%         C1
%         P_B
...
%     end
    ...
    Q0=l0+rho*C_opt_multi_comparison(L,l0*alpha+delta_l,X_exploit,count-1,N,q_ll);
else
    Qi=lm;
    Q0=l0;
end
