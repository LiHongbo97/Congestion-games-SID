function [ output_args ] = mechanism_comparison(input_args)
...
for i=1:9
    q_ll(i)=(1-0.1*(i-1)*1.1)/(1-0.1*(i-1));
%     bar_x(i)=(1-q_ll(i))/(2-q_ll(i)-q_hh)
end
...
for k=1:N
    ...
    for q=1:20
        ...
        l0_info=100;
        l0_info_i=100;
        C_m(k,q)=0;C_opt(k,q)=0;C_info(k,q)=0;C_info_i(k,q)=0;
...
%             L_info_i(l)=101+0.1*(l-1);
%             X_m(l)=0.1+0.01*(l-1);
%             X_opt(l)=0.1+0.01*(l-1);
%             X_info(l,1)=0.1+0.01*(l-1);
%             X_info_i(l,1)=0.1+0.01*(l-1);
%         end
        for i=1:25
            % myopic decision
            lm_m=min(L_m);
            ...
            if lm_m<l0_m
                ...
                for j=1:3 % probability transition
                    ...
                    X_m(j)=X_m(j)*q_hh+(1-X_m(j))*(1-q_ll(k));
                end
                L_m(m)=L_m(m)+delta_l;
            else
                %         0
            ...
            [C_1,pi]=C_opt_multi(L_opt,l0_opt,X_opt,6,3,q_ll(k));
            %     lm_opt
            %     l0_opt
            %     pi
            if pi==1
                ...
                for j=1:3 % probability transition
                    X_opt(j)=X_opt(j)*q_hh+(1-X_opt(j))*(1-q_ll(k));
            ...
            x_bar=(1-q_ll(k))/(2-q_ll(k)-q_hh);
            lm_info=min(L_info);
            m=find(L_info==min(L_info));
%             X_info
%             X_info(:,i)
            [C_1,pi]=C_opt_multi(L_info,l0_info,X_info(:,i),6,3,q_ll(k));
...
%                 end
            ...
            if info_pi==1
                ...
                for j=1:3 % probability transition
                    X_info(j,i+1)=X_info(j,i)*q_hh+(1-X_info(j,i))*(1-q_ll(k));
...
median=[];
for i=1:N
    median(1)=med_C_opt(i);
    median(2)=med_C_info(i);
    median=sort(median,'descend');
    med_C_opt(i)=median(2);
    med_C_info(i)=median(1);
...
kx=0:0.1:0.8;
IR_m=sort(IR_m,'descend');IR_info1=sort(IR_info1,'descend');
% IR_m
% IR_info1
% IR_m_1=[4.6228 3.4087 1.4056 1.4044 1.2538 1.1948 1.1105 1.0876 1.0539];
% IR_info1_1=[1.5815 1.3889 1.1007 1.0953 1.0523 1.0379 1.0345 1.0312 1.0071]; 

% Note that the following two sets are obtained by precalculating alpha_h=10 in line 16
IR_m_1=[1.6484 4.0195 1.9376 1.1515 1.2331 1.1034 1.1231 1.1609 1.0293];
IR_info1_1=[1.6299 1.0596 1.4951 1.0135 1.0411 1.0479 1.0737 1.0601 1.0048]; 
